...
function addSchedule($pdo, $startTime, $endTime, $username, $status) {
    $stmt = $pdo->prepare("INSERT INTO comlab3 (schedule_start_time, schedule_end_time, username, status) VALUES (?, ?, ?, ?)");
    $stmt->execute([$startTime, $endTime, $username, $status]);
}

function cancelSchedule($pdo, $id) {
    $stmt = $pdo->prepare("UPDATE comlab3 SET status = 'Cancelled' WHERE id = ?");
    return $stmt->execute([$id]);
}

function updateOngoingSchedules($pdo) {
    $stmt = $pdo->prepare("UPDATE comlab3 SET status = 'On Going' WHERE schedule_start_time <= NOW() AND schedule_end_time >= NOW() AND status != 'On Going'");
    $stmt->execute();
}

function fetchAllSchedules($pdo) {
    $stmt = $pdo->prepare("SELECT * FROM comlab3");
    $stmt->execute();
